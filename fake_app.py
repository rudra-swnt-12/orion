...
REQUEST_LATENCY = Gauge('fake_app_request_latency_seconds', 'Fake request latency in seconds')
ERRORS_TOTAL = Counter('fake_app_errors_total', 'Total fake errors encountered', ['error_type'])
...
def run_chaos_loop():
    ...
    while True:
        try:
            ...
            log.info(f"OK: Service running normally. CPU: {cpu:.2f}%, Latency: {lat:.3f}s")
            ...
            if random.random() < 0.1:
                ...
                log.error("FATAL: Database connection limit reached! Cannot connect to payment-db.")
