...
from src.agent.state import OrionState, Hypothesis
...
async def run_tool(tool_call: Dict[str, Any]) -> str:
    """
    ...
    try:
        ...
        async with McpClient(transport) as client:
            log.info(
                f"Calling tool: {tool_call.tool_name} with input: {tool_call.tool_input}"
            )

            # 3. Run the tool and get the result
            result = await client.call_tool(tool_call.tool_name, tool_args)
...
def synthesize_conclusion(state: OrionState) -> OrionState:
    ...
    """
    log.info("--- 🔬 SYNTHESIZING Conclusion ---")
